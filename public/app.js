...
$('register-form').addEventListener('submit',async e=>{e.preventDefault();if(busy)return;lookupSequence++;busy=true;$('lookup-form').querySelector('button').disabled=true;$('forget').disabled=true;$('register-button').disabled=true;const values=Object.fromEntries(new FormData(e.currentTarget));if(!pendingToken){pendingToken=Array.from(crypto.getRandomValues(new Uint8Array(32)),b=>b.toString(16).padStart(2,'0')).join('');save('bike-pending-token',pendingToken);}showCode(pendingToken);message('register-message','正在保存登記，請保留查詢碼…');
 ...
 finally{busy=false;$('lookup-form').querySelector('button').disabled=false;$('forget').disabled=false;$('register-button').disabled=!summary||summary.total===null||Date.now()-lastSuccess>45000;}});
