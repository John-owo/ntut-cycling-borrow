...
async function lookup(token,manual=false,seq=++lookupSequence){if(manual)manualLookup=seq;try{const data=await api('/api/me',{token});if(seq!==lookupSequence)return; if(token===pendingToken){pendingToken='';save('bike-pending-token','');}savedToken=token;save('bike-query-token',token);if(manual||document.activeElement!==$('lookup-token'))$('lookup-token').value=token;personal(data.record);renderSummary(data.summary);message('lookup-message','已取得最新狀態。');if(manual)showCode(token);}catch(e){if(seq!==lookupSequence)return;message('lookup-message',`查詢失敗：${e.message}${$('personal-result').hidden?'':' 下方為上次查詢結果。'}`,true);if(e.status===404){$('personal-result').hidden=true;}throw e;}finally{if(manualLookup===seq)manualLookup=0;}}
// Manual lookups and registration invalidate older reads, including public summaries.
async function refresh(){
 if(refreshing||busy||manualLookup)return;
 refreshing=true;const seq=++lookupSequence;
 const publicSummary=async()=>{const result=await api('/api/summary');if(seq===lookupSequence)renderSummary(result);};
 try{if(savedToken&&!pendingToken){try{await lookup(savedToken,false,seq);}catch{if(seq===lookupSequence)await publicSummary();}}else await publicSummary();}
 catch(e){if(seq===lookupSequence)stale(e.message);}finally{refreshing=false;}
}
$('refresh').addEventListener('click',refresh);
$('register-form').addEventListener('submit',async e=>{e.preventDefault();if(busy)return;if(!termsRead||!agree.checked){message('register-message','請先讀完借車須知並勾選確認。',true);return;}updateRegisterButton();if($('register-button').disabled)return;lookupSequence++;manualLookup=0;busy=true;$('lookup-form').querySelector('button').disabled=true;$('forget').disabled=true;updateRegisterButton();const values=Object.fromEntries(new FormData(e.currentTarget));if(!pendingToken){pendingToken=Array.from(crypto.getRandomValues(new Uint8Array(32)),b=>b.toString(16).padStart(2,'0')).join('');save('bike-pending-token',pendingToken);}showCode(pendingToken);message('register-message','正在保存登記，請保留查詢碼…');
