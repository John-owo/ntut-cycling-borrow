...
async function refresh(){if(refreshing||busy)return;refreshing=true;try{if(savedToken){try{await lookup(savedToken);}catch{renderSummary(await api('/api/summary'));}}else renderSummary(await api('/api/summary'));}catch(e){stale(e.message);}finally{refreshing=false;}}
