...
export function createApp({ dbPath = resolve('data/bikes.sqlite'), origins = [], publicDir = resolve('public'), rateLimit = 120 } = {}) {
  ...
  const server = http.createServer(async (req,res) => {
    ...
    try {
      ...
      else if(req.method==='GET'&&path==='/api/admin/records') result=transaction(()=>{const s=summary();return {opening:opening(),summary:s,records:db.prepare('SELECT * FROM records ORDER BY id').all().map(r=>decorate(r,s)),audit:db.prepare('SELECT * FROM audit ORDER BY id DESC').all()};});
