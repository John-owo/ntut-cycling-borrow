...
export function createApp({ dbPath = resolve('data/bikes.sqlite'), origins = [], hosts = [], publicDir = resolve('public'), rateLimit = 120, loginLimit = 10, registerLimits = { minute: 15, hour: 90, day: 300, client: 10 } } = {}) {
  ...
  const server = http.createServer(async (req,res) => {
    ...
    res.setHeader('Content-Security-Policy',"default-src 'self'; connect-src 'self' https://*.supabase.co http://127.0.0.1:* http://localhost:*; script-src 'self'; style-src 'self'; img-src 'self' data: blob:; object-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'");
    ...
    try {
      ...
      if(!path.startsWith('/api/')) { const files={'/':'index.html','/index.html':'index.html','/admin.html':'admin.html','/app.js':'app.js','/admin.js':'admin.js','/api.js':'api.js','/i18n.js':'i18n.js','/translations.js':'translations.js','/style.css':'style.css','/styles.css':'styles.css','/config.js':'config.js','/favicon.svg':'favicon.svg','/ntut-club-logo.png':'ntut-club-logo.png'}; const file=files[path]; if(req.method!=='GET'||!file) fail(404,'找不到頁面'); let contents; try{contents=readFileSync(resolve(publicDir,file));}catch{fail(404,'找不到頁面');}res.writeHead(200,{'Content-Type':file.endsWith('.html')?'text/html; charset=utf-8':file.endsWith('.css')?'text/css; charset=utf-8':file.endsWith('.png')?'image/png':file.endsWith('.svg')?'image/svg+xml':'text/javascript; charset=utf-8'});res.end(contents);return; }
      const now=Date.now(); limit(`${req.socket.remoteAddress}:${path}`,rateLimit,now); if(path==='/api/admin/login') limit(`login:${req.socket.remoteAddress}`,loginLimit,now);
