...
const fill=async(id,name)=>{await page.locator('#borrow-terms-text').evaluate(el=>{el.scrollTop=el.scrollHeight;});await page.locator('#terms-agree:enabled').waitFor();await page.locator('#terms-agree').check();await page.locator('[name=studentId]').fill(id);await page.locator('[name=name]').fill(name);await page.locator('[name=purpose]').selectOption('group_ride');await page.locator('[name=contact]').fill('synthetic-only');await page.locator('#register-button').click();};
try{
